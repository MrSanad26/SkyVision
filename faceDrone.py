import cv2, cvzone, pickle, face_recognition, numpy as np, pymongo as mg, json, datetime, os
...
# Connect to MongoDB
client = mg.MongoClient('localhost',27017).get_database('skyvision').get_collection('wantedPeople')

# Connect to drone and open the control pad
kp.init()
me = tello.Tello()
me.connect()
me.streamon()
...
# import banners
banner = [cv2.imread('static/1.png'),cv2.imread('static/2.png'),cv2.imread('static/3.png'),cv2.imread('static/4.png')]
mode   = 0
...
# declare wnated person id that we will use it to pull from Database 
id = ""
...
# Changing wanted persons counter
arrowCounter = 0
...
while True:
    ...
    for encodeFace, faceLoc in zip(encodeCurFrame, faceCurFrame):
        ...
        if matches[matchIndex]:
            ...
            cvzone.putTextRect(bg,info[id]["Name"],(x1 + x ,y1 + y),1,1,(255,255,255),(0,255,0),cv2.FONT_HERSHEY_COMPLEX_SMALL)
            sPX, sPY = 400, 950
            # Number of wanted people front the camera
            cv2.putText(bg,f'Wanted People in the Camera: {len(info)}'                ,(sPY,50 ),cv2.FONT_HERSHEY_COMPLEX_SMALL,0.7,(255,255,255),1)
            # Inserting person info
            cv2.putText(bg,f'ID: {listInfo[arrowCounter]["ID"]}'                      ,(sPY,sPX+00 ),cv2.FONT_HERSHEY_COMPLEX_SMALL,0.8,(255,255,255),1)
            cv2.putText(bg,f'Name: {listInfo[arrowCounter]["Name"]}'                  ,(sPY,sPX+40 ),cv2.FONT_HERSHEY_COMPLEX_SMALL,0.8,(255,255,255),1)
            cv2.putText(bg,f'Severity: {listInfo[arrowCounter]["Severity"]}'          ,(sPY,sPX+80 ),cv2.FONT_HERSHEY_COMPLEX_SMALL,0.8,(255,255,255),1)
            cv2.putText(bg,f'Last Time: {listInfo[arrowCounter]["Last Time"]}'        ,(sPY,sPX+120),cv2.FONT_HERSHEY_COMPLEX_SMALL,0.8,(255,255,255),1)
            cv2.putText(bg,f'Last Location:'                                          ,(sPY,sPX+160),cv2.FONT_HERSHEY_COMPLEX_SMALL,0.8,(255,255,255),1)
            cv2.putText(bg,f'{listInfo[arrowCounter]["Last Location"]}'               ,(sPY,sPX+200),cv2.FONT_HERSHEY_COMPLEX_SMALL,0.8,(255,255,255),1)
            # Wanted Persons Photo
            bg[100:100+216, 995:995+216] = cv2.imread(f'wantedPersons/{listInfo[arrowCounter]["ID"]}.png')
